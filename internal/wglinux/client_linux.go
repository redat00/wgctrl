...
func NewNetNS(netns int) (*Client, bool, error) {
	c, err := genetlink.Dial(&netlink.Config{NetNS: netns})
	if err != nil {
	...
	cl, ok, err := initClient(c)
	if err != nil || !ok {
		return cl, ok, err
	}

	if netns != 0 {
		cl.interfaces = func() ([]string, error) { return rtnlInterfacesNetNS(netns) }
	}

	return cl, true, nil
}

func rtnlInterfacesNetNS(netns int) ([]string, error) {
	c, err := netlink.Dial(unix.NETLINK_ROUTE, &netlink.Config{NetNS: netns})
	if err != nil {
		return nil, fmt.Errorf("wglinux: failed to open rtnetlink socket: %v", err)
	}
	defer c.Close()
