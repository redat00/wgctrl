...
import (
	...
	"github.com/redat00/wgctrl"
	"github.com/redat00/wgctrl/wgtypes"
)

func newClient(netns string) (*wgctrl.Client, error) {
	if netns == "" {
		return wgctrl.New()
	}
	...
	if !strings.Contains(netns, "/") {
		path = "/run/netns/" + netns
	}

	f, err := os.Open(path)
	if err != nil {
		return nil, fmt.Errorf("failed to open network namespace %q: %w", netns, err)
	}
	defer f.Close()

	return wgctrl.NewWithNetNS(int(f.Fd()))
}

func main() {
	netns := flag.String("netns", "", "")

	flag.Parse()

	c, err := newClient(*netns)
	if err != nil {
